    ...
    <div class="container ps-0 pe-5">
        ...
        <form action="{{ URL('/search-results') }}" class="ms-5 form-blur-border">
            <div class="row bg-whiter justify-content-between text-start py-3 px-5 m-0 box-sizing form-border FormBorder-1"
                ...
                <div class="col-sm-4 px-0 border-end border-end0">
                    <label class="mb-0 label-none my-font-size"><b>What have you got planned?</b></label>
                    <input type="text" id="planCatagories-1" class="form-control px-0" placeholder="Select your plan">
                </div>
                <div class="col-sm-4 border-end border-end0 px-0 px-sm-3 my-3 my-sm-0">
                    <label class="mb-0 label-none font-size"><b>Where?</b></label>
                    <input type="text" class="form-control px-0" id="location-1" placeholder="Enter your Location">
                </div>
                <div class="col-sm-3 border-end0 pe-0 px-0 px-sm-3 mb-3 mb-sm-0">
                    <label class="mb-0 label-none font-size"><b>When?</b></label>
                    <input type="text" class="form-control input py-2 px-0" placeholder="Anytime" id="selectedDate-1">
                </div>
                <div class="col-sm-1 text-end px-0 my-auto">
            ...
            <div class="row w-100 bg-white calendarAndTimePicker-1" id="calendarAndTimePicker-1">
                ...
                <div class="col-6 dialog" id="dialog">
                    ...
                    <div class="form-container mt-7 me-6" align="center">
                        ...
                        <button type="button" class="clear_button" id="clear-button1">Clear</button>
                    </div>
                </div>
            </div>
        </form>
...
<div style="margin-top: 65px!important;" class="landing_background1 height-img">
    <div class="container ps-0 pe-5">
        ...
        <form action="{{ URL('/search-results') }}" class="ms-5 form-blur-border">
            <div class="row bg-whiter justify-content-between text-start py-3 px-5 m-0 box-sizing form-border"
                id="form-border2">

                <div class="col-sm-4 px-0 border-end border-end0">
                    <label class="mb-0 label-none my-font-size"><b>What have you got planned?</b></label>
                    <input type="text" id="planCatagories-2" class="form-control px-0" placeholder="Select your plan">
                </div>
                <div class="col-sm-4 border-end border-end0 px-0 px-sm-3 my-3 my-sm-0">
                    <label class="mb-0 label-none font-size"><b>Where?</b></label>
                    <input type="text" class="form-control px-0" id="location2" placeholder="Enter your Location">
                </div>
                <div class="col-sm-3 border-end0 pe-0 px-0 px-sm-3 mb-3 mb-sm-0">
                    <label class="mb-0 label-none font-size"><b>When?</b></label>
                    <input type="text" class="form-control input py-2 px-0" placeholder="Anytime" id="selectedDate2">
                </div>
                <div class="col-sm-1 text-end px-0 my-auto">
                    <button type="submit" class="btn btn-color rounded text-end button ms-auto d-none d-sm-block"><i
                            class="fa fa-search text-white"></i></button>
...
<script>
    $(document).ready(function() {
        ...
        $(document).click(function(event) {
            var target = $(event.target);
            if (!target.closest('#calendarAndTimePicker-1').length && !target.is('#selectedDate-1')) {
                $('#calendarAndTimePicker-1').hide();
    ...
    function init_calendar_1(date) {
        $(".tbody-tb-1").empty();
        var calendar_days = $(".tbody-tb-1");
        var month = date.getMonth();
        var year = date.getFullYear();
        var day_count = days_in_month_1(month, year);
        var row = $("<tr class='table-row'></tr>");
        var today = date.getDate();
        date.setDate(1);
        var first_day = date.getDay();
        for (var i = 0; i < 35 + first_day; i++) {
            ...
            if (i < first_day || day > day_count) {
                ...
                var curr_date = $("<td class='table-date'>" + day + "</td>");
                var events = check_events_1(day, month + 1, year);
                if (today === day && $(".active-date").length === 0) {
                    curr_date.addClass("active-date");
                }
                curr_date.click({
    ...
    function date_click_1(event) {
        ...
        $(this).addClass("active-date");
        const day = $(this).text();
        const month = $('.month-1.active-month').text();
        const year = $('.year-1').text();
        var fullDate = month + ' ' + day + ', ' + year;
        $('#hiddenDate-1').val(fullDate);
    ...
    function month_click_1(event) {
        ...
        $(".active-month").removeClass("active-month");
        $(this).addClass("active-month");
        var new_month = $(".month-1").index(this);
        date.setMonth(new_month);
        init_calendar_1(date);
    }
    ...
    function check_events_1(day, month, year) {
        ...
        for (var i = 0; i < event_data_1["events"].length; i++) {
            var event = event_data_1["events"][i];
            if (
                event["day"] === day &&
                event["month"] === month &&
                event["year"] === year
    ...
    const $startTimeInput = $("#start_time_1");
    const $endTimeInput = $("#end_time_1");
    const $appendedValue = $('#selectedDate-1');

    $startTimeInput.on("change", updateAppendedValue_1);
    $endTimeInput.on("change", updateAppendedValue_1);

    function updateAppendedValue_1() {
        const selectedDate = $("#hiddenDate-1").val();
        const selectedStartTime = $startTimeInput.val();
        const selectedEndTime = $endTimeInput.val();
...
<script>
    $(document).ready(function() {
            ...
            function hideDropdown(resultsDiv, inputDiv) {
                ...
                inputDiv.css("border-bottom-left-radius", "13px !important").css("border-bottom-right-radius", "13px !important");
            }
                    ...
                    autocompleteService.getPlacePredictions({
                        ...
                        if (status === google.maps.places.PlacesServiceStatus.OK) {
                            ...
                            predictions.forEach(function(prediction) {
                                var addressItem = $('<div class="text-start p-3 drop-address-result">').text(prediction.description);
                                addressItem.on('click', function() {
                                    locationInput.val(prediction.description);
                                    addressDropdown.empty();
                                    locationInput.parent().removeClass("data-appended");
                                    locationInput.parent().css("border-bottom-left-radius", "13px !important").css("border-bottom-right-radius", "13px !important");
                                });
                                addressDropdown.append(addressItem);
                                locationInput.parent().addClass("data-appended");
                                locationInput.parent().css("border-bottom-left-radius", "0px !important").css("border-bottom-right-radius", "0px !important");
                            });
                        }
                    });
                });
            ...
            setupLocationAutocomplete("#location1", "#addressDropdown1");
        });
</script>
    ...
    $(document).ready(function() {
            ...
            function setupSearch(inputId, resultsId) {
                ...
                searchInput.on("input", function() {
                    ...
                    searchResults.html("").css("border-bottom-left-radius", "13px").css("border-bottom-right-radius", "13px");

                    if (inputValue.length > 0) {
                        filteredCategories.slice(0, maxResultsToShow).forEach(category => {
                            const div = $("<div class='search-drop-content text-start ps-5'>").text(category);
                        ...
                        if (filteredCategories.length === 0) {
                            ...
                            const para = $("<p>").text("We don't recognize that activity. Try describing it differently or ");
                            const anchor = $("<a class='modal-effect' data-bs-target='#popular-catagories-modal' data-bs-toggle='modal'>")
                    ...
                    if (inputValue.length > 0) {
                        ...
                        $(inputId).parent().css("border-bottom-left-radius", "13px !important").css("border-bottom-right-radius", "13px !important");
                    }
                });
            ...
            setupSearch("#planCatagories", "#searchResults");
            setupSearch("#planCatagories-1", "#searchResults-1");
            setupSearch("#planCatagories-2", "#searchResults-2");
        });
</script>
<!-- OWL CAROUSEL JS-->
