                            ...
                            <table id="example2"
                                ...
                                <thead class="table_head_row">
                                    <tr role="row" class="text-center">
                                        ...
                                        <th class="sorting text-white p-4" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1"
                                            aria-label="Salary: activate to sort column ascending">{{ lang('Type') }}
                                        </th>
                                        {{-- <th class="sorting text-white p-4" tabindex="0" aria-controls="example2"
                                        ...
                                        <th class="sorting text-white p-4" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1"
                                            aria-label="Salary: activate to sort column ascending">{{ lang('Status') }}
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if (isset($orders))
                                        @foreach (@$orders as $pendingBooking)
                                            <tr class="text-center">
                                                ...
                                                <td>{{ lang(
                                                    Str::limit(
                                                        @$pendingBooking->type == 'space'
                                                            ? @$pendingBooking->space->address
                                                            : (@$pendingBooking->type == 'entertainment'
                                                                ? @$pendingBooking->entertainment->address
                                                                : @$pendingBooking->service->address),
                                                        30,
                                                        $end = '...',
                                                    ),
                                                ) }}
                                                ...
                                                <td>{{ lang(
                                                    @$pendingBooking->type == 'space'
                                                        ? @$pendingBooking->space->spaceType->type
                                                        : (@$pendingBooking->type == 'entertainment'
                                                            ? @$pendingBooking->entertainment->title
                                                ...
                                                <td class="text-end">
                                                    @if (@$pendingBooking->status == 3)
                                                        <span disabled
                                                            class="tag tag-rounded tag-icon tag-red">{{ lang('Declined') }}</span>
                                                    @elseif (@$pendingBooking->status == 2)
                                                        <span disabled
                                                            class="tag tag-rounded tag-icon tag-green">{{ lang('Accepted') }}</span>
                                                    @elseif(@$pendingBooking->status == 1)
                                                        <span disabled
                                                            class="tag tag-rounded tag-icon tag-warning">{{ lang('Pending') }}</span>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
        ...
        function cancelBooking(url) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    var route_name = "{{ route('cancel-booking', ['id' => 'route_id']) }}".replace('route_id',
                        res.id);
                    $('#amount_perc').html(res.amount_perc +
                        '% will be deducted from your total booking price.');
